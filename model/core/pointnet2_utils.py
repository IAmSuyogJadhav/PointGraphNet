# Modified by: IAmSuyogJadhav, 31-01-2023
# Original Author: yanx27, 30-08-2022
...
def index_points(points, idx, dtype=torch.long):
    """
...
def farthest_point_sample(xyz, npoint, dtype=torch.long):
    """
    Input:
    ...
    device = xyz.device
    B, N, C = xyz.shape
    centroids = torch.zeros(B, npoint, dtype=dtype).to(device)
    distance = torch.ones(B, N).to(device) * 1e10
    farthest = torch.randint(0, N, (B,), dtype=dtype).to(device)
    batch_indices = torch.arange(B, dtype=dtype, device=device).to(device)
    for i in range(npoint):
        centroids[:, i] = farthest
        centroid = xyz[batch_indices.long(), farthest.long(), :].view(B, 1, 3)
...
def query_ball_point(radius, nsample, xyz, new_xyz, dtype=torch.long):
    """
    Input:
...
class PointNetSetAbstractionMsg(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        for i, radius in enumerate(self.radius_list):
            ...
            group_idx = query_ball_point(radius, K, xyz, new_xyz, dtype=self.dtype)
            grouped_xyz = index_points(xyz, group_idx, dtype=self.dtype)
            grouped_xyz -= new_xyz.view(B, S, 1, C)
            if points is not None:
                grouped_points = index_points(points, group_idx, dtype=self.dtype)
