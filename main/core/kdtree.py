...
class PointsGraph:
    ...
    def __init__(
        ...
        self.points_torch = (
            None  # Points in a torch tensor, used externally for training and such
        )
        ...
        if not infer_mode:
            assert (
                abs(self.df.theta.max() - self.angle_max) <= 100
            ) and self.df.theta.max() <= 2 * self.angle_max, f"Theta max is {self.df.theta.max()}, but should be <= {self.angle_max} (Angle max set in __init__)"
            assert (
                abs(self.df.phi.max() - self.angle_max) <= 100
            ) and self.df.phi.max() <= 2 * self.angle_max, f"Phi max is {self.df.phi.max()}, but should be <= {self.angle_max} (Angle max set in __init__)"
    ...
    def reset_tree(self):
        ...
        if TORCH_INSTALLED and not self.no_torch:
            if not self.infer_mode:
                self.points_torch = (
                    torch.from_numpy(self.points).float().to(self.device)
                )
            else:
                self.points_torch = (
                    torch.from_numpy(self.points).half().to(self.device)
                )  # debug
    ...
    def get_gt(self, strategy=1, K=None) -> tuple[np.ndarray]:
        ...
        if self.gt is None:
            if strategy == 1:
                self.df["is_noise"] = (
                    self.df["label"].str.startswith(self.noise_label)
                ).astype(int)
                tpn_gt = self.df[["theta", "phi", "is_noise"]].to_numpy() / [
                    self.angle_max,
                    self.angle_max,
                    1,
                ]  # Normalize to 0-1
    ...
    def get_k_nearest_all(self, k=5, include_self=True):
        ...
        if (
            self.neighbor_idxs is None
            or self.neighbor_dists is None
            or self.last_K != k
        ):
            ...
            dists, idxs = self.tree.kneighbors(
                self.points, n_neighbors=k + 1, n_jobs=self.n_jobs
            )  # +1 to include the point itself # pynaoflann is faster than sklearn
            idxs = idxs.astype(
                np.int32
            )  # Convert uint64 to int32 for compatibility with PyTorch
            self.neighbor_dists, self.neighbor_idxs, self.last_K = (
                dists,
                idxs,
                k,
            )  # Cache the results
    ...
    def get_k_nearest(self, idxs, k=5, include_self=True):
        ...
        dists, idxs = self.tree.kneighbors(
            points, n_neighbors=k + 1, n_jobs=self.n_jobs
        )  # pynaoflann is faster than sklearn
        idxs = idxs.astype(
            np.int32
        )  # Convert uint64 to int32 for compatibility with PyTorch
    ...
    def reduce_points(self, r=2):
        ...
        neighbor_idxs = self.tree.radius_neighbors(
            self.points, radius=r, return_distance=False, n_jobs=self.n_jobs
        )  # pynaoflann is faster than sklearn
        ...
        point_idxs.sort(
            key=lambda i: len(neighbor_idxs[i]), reverse=False
        )  # Sort the indices by the length of the list (shortest first)  # DEBUG
        to_remove = set()
    ...
    def _compute_edges(self, k=5):
        node_ends = np.vstack(
            kneighbors_graph(self.points, n_neighbors=k, include_self=False)
            .tolil()
            .rows
        )
    ...
    def visualize(self):
        ...
        edge_xyz = np.array(
            [(self.points[u], self.points[v]) for u, v in self.graph.edges()]
        )
...
# @profile
def load_data(path):
    if path.endswith(".mat") or path.endswith(
        ".dat"
    ):  # For files obtained from testSTORM
        ...
        try:
            ...
            df.rename(
                columns={"x [nm]": "x", "y [nm]": "y", "z [nm]": "z"}, inplace=True
            )
...
def main(k, filepath, last_idx, visualize=False, reduce_radius=None):
    ...
    if visualize:
        ...
        grapher.build_and_save(
            f'./graph_{os.path.basename(filepath).split(".")[0]}_k={k}_last_idx={last_idx}_reduce_radius={reduce_radius}.graphml',
            k=k,
        )
