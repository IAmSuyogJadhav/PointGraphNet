...
ax.scatter(points['x'], points['y'], points['z'], c='black', alpha=0.8, marker='.', s=0.1)
ax.set_xlabel('x'); ax.set_ylabel('y'); ax.set_zlabel('z')

# Plot the cross section normal vectors and corresponding orthogonal vectors
step = 25
ax.quiver(curve[::step, 0], curve[::step, 1], curve[::step, 2], normals[::step, 0], normals[::step, 1], normals[::step, 2], color='red', label='Cross-section normals', length=500)
ax.quiver(curve[::step, 0], curve[::step, 1], curve[::step, 2], orth0[::step, 0], orth0[::step, 1], orth0[::step, 2], color='blue', label='Orthogonal vectors', length=500)
ax.quiver(curve[::step, 0], curve[::step, 1], curve[::step, 2], orth1[::step, 0], orth1[::step, 1], orth1[::step, 2], color='green', label='Orthogonal vectors', length=500)
ax.legend()

ax.set_xlim(-max_xy, max_xy); ax.set_ylim(-max_xy, max_xy); ax.set_zlim(-max_xy, max_xy)
...
fig = plt.figure(figsize=(10, 10))
ax = fig.add_subplot(111, projection='3d')
ax.scatter(points['x'], points['y'], points['z'], c=points['z'], alpha=0.5, marker='.', label='Surface points')
ax.set_xlabel('x'); ax.set_ylabel('y'); ax.set_zlabel('z')

# Plot the surface normal vectors
step = 5
ax.quiver(points['x'][::step], points['y'][::step], points['z'][::step], points['nx'][::step], points['ny'][::step], points['nz'][::step], color='blue', label='Surface normals', length=200, alpha=0.3)
ax.legend()
