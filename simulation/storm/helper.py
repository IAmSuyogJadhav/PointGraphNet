...
def add_imprecision(points: pd.DataFrame, shift_radius: float = 20) -> pd.DataFrame:
    ...
    points_shifted[["x", "y", "z"]] += np.random.uniform(
        -shift_radius, shift_radius, size=(points.shape[0], 3)
    )
...
def get_foreground_noise(
    ...
    points_noisy = points_noisy.sample(
        frac=noise_percent, replace=True
    )  # Sample with replacement, to allow for multiple noisy points per original point
    ...
    points_noisy[["x", "y", "z"]] += random_uniform_discontinuous(
        -noise_radius_max,
        -noise_radius_min,
        noise_radius_min,
        noise_radius_max,
        size=(points_noisy.shape[0], 3),
    )
...
def get_background_noise(
    points: pd.DataFrame, noise_percent: float, max_xy: float, zlow: float, zhigh: float
) -> pd.DataFrame:
...
def simulate_thunderstorm(
    ...
    noise_fg = get_foreground_noise(
        points=points,
        noise_percent=noise_percent_fg,
        noise_radius_min=noise_radius_min,
        noise_radius_max=noise_radius_max,
    )

    # Step 4: Add random noise to the background as an additional source of noise
    noise_bg = get_background_noise(
        points=points,
        noise_percent=noise_percent_bg,
        max_xy=max_xy,
        zlow=zlow,
        zhigh=zhigh,
    )

    points = pd.concat([points, noise_fg, noise_bg], ignore_index=True).reset_index(
        drop=True
    )
    return points
