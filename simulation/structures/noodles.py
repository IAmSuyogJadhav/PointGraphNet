...
def _get_cross_section_normals(points: np.ndarray):
    ...
    normals[0] = -normals[
        0
    ]  # flip the first normal to point towards the end of the curve
...
def _get_noodle_surface(
    ...
    **kwargs,
    ...
    area_caps = (
        4 * np.pi * (width / 2) ** 2
    )  # Surface area of the two hemispherical caps

    points_body = math.ceil(
        area_body * density
    )  # Number of points on the cylinder body (total)

    points_per_circle = math.ceil(
        points_body / len(curve)
    )  # Number of points on the cylinder body (per circle)
    points_caps = math.ceil(
        area_caps * density
    )  # Number of points on the hemispherical caps
    points_per_cap = math.ceil(
        points_caps / 2
    )  # Number of points on each hemispherical cap

    npoints_orig = (
        points_body + points_caps
    )  # Total number of points, it is made sure we don't exceed this

    # Oversample the curve if needed
    if oversample is not None:
        assert isinstance(oversample, float) or isinstance(
            oversample, int
        ), "Oversample must be a float or an int"
    ...
    orth0 = np.hstack(
        [
            np.zeros((len(normals), 1)),
            -normals[:, 2].reshape(-1, 1),
            normals[:, 1].reshape(-1, 1),
        ]
    )
    ...
    orth0 = np.where(  # If the norm of the vector is 0, replace it with backup vector 1
        np.linalg.norm(orth0, axis=1).reshape(-1, 1) > 0, orth0, orth1
    )

    orth0 = np.where(  # If the norm of the vector is still 0, replace it with backup vector 2
        np.linalg.norm(orth0, axis=1).reshape(-1, 1) > 0, orth0, orth2
    )
    ...
    orth0 = orth0 / np.linalg.norm(orth0, axis=1).reshape(
        -1, 1
    ...
    for i, point in enumerate(curve):  # For each point on the curve
        t = np.random.uniform(0, 2 * np.pi, points_per_circle)[
            ..., None
        ]  # Random thetas; N, 1
        ...
        if with_normals:  # If we want to return the normals, calculate them
            ...
            nxyz = (
                xyz - point
            )  # Normal vector is the vector pointing from the center to the point on the circle
            nxyz = nxyz / np.linalg.norm(nxyz, axis=1).reshape(-1, 1)  # Normalize
            phi = np.arctan2(nxyz[:, 1], nxyz[:, 0]).reshape(
                -1, 1
            )  # Calculate phi (angle from x-axis)
            theta = np.arccos(nxyz[:, 2]).reshape(
                -1, 1
    ...
    circle_points = np.vstack(
        circle_points
    )  # Stack all the points together. N, 3 or N, 8
    ...
    sphere_points = [
        _get_sphere(
            point,
            r,
            CAP_OVERSAMPLING * points_per_cap,
            zlow,
            zhigh,
            max_xy,
            with_normals=with_normals,
            use_gpu=points_per_cap > 50,  # Use GPU only if there are a lot of points
        )
    ...
    hemisphere_points = []
    for normal, sphere_ps, center in zip(
        [normals[0], normals[-1]], sphere_points, [curve[0], curve[-1]]
    ):
        ...
        hemisphere_points.append(
            np.array(keep_points)[:points_per_cap]
        )  # Keep only the desired number of points

    # Remove empty lists (if any)
    hemisphere_points = np.vstack(
        hemisphere_points
    )  # Stack all the points together. N, 3 or N, 8
    ...
    if len(to_stack) == 0:
        raise ValueError(
            'No points generated. Check the parameters or try the "oversample" parameter.'
        )
    elif len(to_stack) == 1:
        points = to_stack[0]
    else:
    ...
    valid = np.all(points[..., :3] > limit_low, axis=1) & np.all(
        points[..., :3] < limit_high, axis=1
    )
...
def get_noodle(
    ...
    # Sanity checks
    assert (
        min_number_of_obj <= max_number_of_obj
    ), "min_number_of_obj must be <= max_number_of_obj"
    assert (
        control_points_lower <= control_points_upper
    ), "control_points_lower must be <= control_points_upper"
    ...
    assert structure in [
        ...
        "microtubules",
        "mito",
    ], "structure must be one of actin, microtubules or mito"
    ...
    for i, curve in enumerate(curves):
        if curve[0] is None:
            ...
            while curves[i][0] is None:  # Retry until a valid curve is generated
                j += 1
                print(
                    f"Some simulations failed. Retrying with different parameters...[{j}/inf]",
                    end="\r",
                )
                curves[i] = _get_bezier(
                    n_control_points=np.random.randint(
                        control_points_lower, control_points_upper + 1
                    ),
                    npoints=npoints,
                    max_xy=max_xy,
                    zlow=zlow,
